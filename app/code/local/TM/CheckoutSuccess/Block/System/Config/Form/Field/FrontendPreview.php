...
<script type="text/javascript">
    ...
    function orderNotFound() {
        var text = '{$this->__("Unfortunatly, we can not find order to preview success page... :(")}'
...
<tr id="row_{$element->getHtmlId()}">
    ...
    <td class="value">
        <p class="{$cssClass}"><input id="cs-preview-order-number" class="input-text" type="text" value="{$this->getLastOrder($this->getStore())}"/><button id="cs-preview-start" onclick="saveAndPreview(this);" class="scalable go" type="button" style="margin-left: 10px;"><span><span><span>{$this->__("Save and Start Preview")}</span></span></span></button>{$msgInsecureConn}</p>
...
HTML;
    ...
    {
        $url[] = rtrim($this->getStore()->getBaseUrl(
